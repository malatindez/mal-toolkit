...
/**
 * @file event.hpp
 * @brief Defines event handling and notification mechanisms.
 */
 ...
 *
 * This macro defines a static constexpr event_name member variable and a name()
 * method for the given event type. The name() method returns the string
 * representation of the event type.
 *
 ...
 */
enum class EventType : uint32_t { None = 0, FilesChanged = 1 };
...
public:
   ...
   */
  [[nodiscard]] uint8_t category_flags() const noexcept {
    return kCategoryFlags;
  };
