...
#define MAL_TOOLKIT_DETAIL_ENDIANNES_ICC 1
#elif defined(__GNUC__)
        result |= uint32_t(CompilerType::kGCC);
#define MAL_TOOLKIT_DETAIL_ENDIANNES_GCC 1
#elif defined(_MSC_VER)
        result |= uint32_t(CompilerType::kMSVC);
#define MAL_TOOLKIT_DETAIL_ENDIANNES_MSVC 1
#endif
        return result;
    }
    ...
    inline uint16_t betole16(uint16_t x)
    {
        ...
        return _bswap16(x);
#elif MAL_TOOLKIT_DETAIL_ENDIANNES_GCC
        return __builtin_bswap16(x);
#elif MAL_TOOLKIT_DETAIL_ENDIANNES_MSVC
