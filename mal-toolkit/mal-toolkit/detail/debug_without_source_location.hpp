...
#include "../debug.hpp"
/**
 * @file debug_without_source_location.hpp
 * @brief Provides utility functions for debugging and assertion handling without source location information.
 */
namespace mal_toolkit
{
     ...
     *
     * In debug mode (MAL_TOOLKIT_DEBUG_ENABLED set to true), an assertion is triggered using
     ...
     *
     * - MAL_TOOLKIT_ASSERT_ABORTS: Determines whether an assertion failure should result in
     ...
     *
     * - MAL_TOOLKIT_ASSERT_LOGS: Indicates whether logging is enabled for assertion failures.
    ...
    inline void Assert(bool value, std::string_view message = "Assert failed")
    {
        ...
        if constexpr (MAL_TOOLKIT_ASSERT_LOGS)
        {
     ...
     *
     * If logging is enabled (MAL_TOOLKIT_ASSERT_LOGS is set to true), the function generates
     ...
     *
     * - MAL_TOOLKIT_DEBUG_ENABLED: Indicates whether debug mode is enabled. In debug mode,
    ...
    inline void AlwaysAssert(bool value, std::string_view message = "Assert failed")
    {
        ...
        {
            if constexpr (MAL_TOOLKIT_ASSERT_ABORTS)
            {
                std::abort();
            }
            else
