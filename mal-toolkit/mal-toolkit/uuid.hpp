...
/**
 * @file uuid.hpp
 * @brief Defines utilities for working with Universally Unique Identifiers (UUIDs).
 */
...
namespace mal_toolkit::uuid
{
    ...
    /**
     * @brief Converts a 128-bits unsigned int to an UUIDv4 string representation using SIMD via
     * AVX2.
     *
     * @param x The 128-bits unsigned int to convert.
     * @param mem The output buffer to store the UUIDv4 string.
     */
    void inline m128itos(__m128i x, char *mem)
    {
        ...
        const __m256i dash_shuffle =
            _mm256_set_epi32(0x0b0a0908, 0x07060504, 0x80030201, 0x00808080, 0x0d0c800b, 0x0a090880,
        ...
        _mm256_storeu_si256((__m256i *)mem, endianness::betole256(resd));
        *(uint16_t *)(mem + 16) = endianness::betole16(_mm256_extract_epi16(res, 7));
        *(uint32_t *)(mem + 32) = endianness::betole32(_mm256_extract_epi32(res, 7));
    }

    /**
     * @brief Converts an UUIDv4 string representation to a 128-bits unsigned int using SIMD via
     * AVX2.
     *
     * @param mem The input UUIDv4 string.
     * @return The converted 128-bits unsigned int.
     */
    __m128i inline stom128i(const char *mem)
    {
        // Remove dashes and pack hex ascii bytes in a 256-bits int
        const __m256i dash_shuffle =
            _mm256_set_epi32(0x80808080, 0x0f0e0d0c, 0x0b0a0908, 0x06050403, 0x80800f0e, 0x0c0b0a09,
                             0x07060504, 0x03020100);

        __m256i x = endianness::betole256(_mm256_loadu_si256((__m256i *)mem));
        x = _mm256_shuffle_epi8(x, dash_shuffle);
        x = _mm256_insert_epi16(x, endianness::betole16(*(uint16_t *)(mem + 16)), 7);
        x = _mm256_insert_epi32(x, endianness::betole32(*(uint32_t *)(mem + 32)), 7);
        ...
        const __m256i shift = _mm256_set_epi32(0x00000000, 0x00000004, 0x00000000, 0x00000004,
    ...
    /**
     * @brief UUIDv4 (random 128-bits) RFC-4122.
     */
    class UUID
    {
    public:
        ...
        /**
         * @brief Constructs an UUID from a 128-bits unsigned int.
         */
        UUID(__m128i uuid) { _mm_store_si128((__m128i *)data, uuid); }

        /**
         * @brief Constructs an UUID from two 64-bit integers.
         */
        ...
        /**
         * @brief Constructs an UUID from a byte string.
         */
        explicit UUID(const std::string &bytes)
        {
            __m128i x = endianness::betole128(_mm_loadu_si128((__m128i *)bytes.data()));
            _mm_store_si128((__m128i *)data, x);
        }
        ...
        {
            __m128i x = endianness::betole128(_mm_load_si128((__m128i *)data));
        ...
        size_t hash() const { return *((uint64_t *)data) ^ *((uint64_t *)data + 8); }
    ...
    public:
        ...
        /**
         * @brief Constructor with a specified seed for the random generator.
         */
        UUIDGenerator(uint64_t seed)
            : generator(new RNG(seed)), distribution(std::numeric_limits<uint64_t>::min(),
                                                     std::numeric_limits<uint64_t>::max())
        {
        }

        /**
         * @brief Constructor with a user-provided random generator.
         */
        UUIDGenerator(RNG &gen)
            : generator(gen), distribution(std::numeric_limits<uint64_t>::min(),
                                           std::numeric_limits<uint64_t>::max())
        {
        }
...
namespace std
{
    template <> struct hash<mal_toolkit::uuid::UUID>
    {
        /**
         * @brief Hashes an UUID instance.
         */
        size_t operator()(const mal_toolkit::uuid::UUID &uuid) const { return uuid.hash(); }
    };
} // namespace std